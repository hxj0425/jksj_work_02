 ...
 * 在toybrowser只挑选其中一部分状态，完成最简单的
 * 
 */

/**
 * 第三步：解析HTML标签
 * html标签分为：开始标签/结束标签/自封毕标签
 * 用FSM来实现HTML的分析
 * 在HTML标准中，已经规定了HTML的状态
 * 在toybrowser只挑选其中一部分状态，完成最简单的
 * 暂时忽略属性
...
/**
 * 第五步：处理属性
 * 属性值分为单引号/双引号/无垠好三种写法，需要较多的状态处理
 * 处理属性的方式跟标签类似
 * 属性结束时，将属性添加到标签Token上
 */
 ...
 * 2.
 * 创建一个元素后，立即计算css
 * 理论上，当分析一个元素时，所有的css规则已经手机好
 * 在真实浏览器中，可能遇到卸载body的style标签需要在css里重新计算，暂时忽略
 * 3.获取父元素
 ...
 * spceificity是个四元组，越靠左边权重越高
 * 一个css规则的spectificity根据包含的简单选择器相加而成
 */
